...
const QUICK = ['chicken', 'paneer', 'salad', 'oats', 'eggs', 'salmon', 'tofu', 'pasta'];
...
export default function AddFood() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null);
    try {
      // With a query -> search; without -> show a tasty default set (high-protein).
      const url = query.trim() ? `${API_BASE}/api/foods/search` : `${API_BASE}/api/foods/recommendations`;
      const params = query.trim() ? { query: query.trim(), number: 12 } : { filter: 'balanced', number: 12 };
    ...
    <AppLayout title="Browse Food">
      <Box sx={{ borderRadius: 5, p: { xs: 3, sm: 4 }, mb: 3, color: '#fff', background: 'linear-gradient(135deg,#16a34a,#84cc16)', boxShadow: '0 24px 50px -22px rgba(22,163,74,.6)' }}>
        ...
        <TextField
          ...
          sx={{ mt: 2.5, maxWidth: 520, bgcolor: '#fff', borderRadius: 999 }}
